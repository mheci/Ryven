...
- `ryven/Bar.qml` — the bar itself (PanelWindow, 34 px, hint text, clock)
...
- Small tweaks (text, colors, spacing, clock): edit `ryven/Bar.qml` directly.
...
- Docs: https://quickshell.org — types: `ShellRoot`, `PanelWindow`,
  `Quickshell.Io` (`Process`), `Quickshell.Hyprland` (native Hyprland IPC:
  workspaces, active window), `Quickshell.Services.SystemTray`, `UPower`.
- Only use imports that appear in the shipped files or that the 0.2 docs
  list; random Qt modules are not available.
- Talk to Hyprland with `hyprctl -j <workspaces|activewindow|...>` (JSON)
  via `Process`, or the native `Quickshell.Hyprland` module.
