...
# no CUDA toolkit (the driver's CUDA runtime libs ship with the driver), no
# Docker, no LIBVA_DRIVER_NAME=nvidia, zswap on / zram off,
...
# KCFLAGS settings into akmodsbuild the way the patched scriptlet does (its
# runuser -c invocation resets the environment), so the build would use
# gcc/ld.bfd against the kernel's clang-LTO CFLAGS.
...
# install_betterbird in common.sh); desktop entry + icon ship ininstall_wl_clip_persist
