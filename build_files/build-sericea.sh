...
dnf5 -y install \
  ...
  libsecret polkit PackageKit mate-polkit \
...
install_priority sway swaybg swayidle swaylock fuzzel quickshell \
...
if [[ -f /usr/lib/os-release ]]; then
  ...
  build_id="ryven-sericea-$(date -u +%Y%m%d%H%M%S)"
  if grep -q '^VARIANT_ID=' /usr/lib/os-release; then
    sed -i 's/^VARIANT_ID=.*/VARIANT_ID="ryven-sericea"/' /usr/lib/os-release
  else
    [[ -z $(tail -c1 /usr/lib/os-release) ]] || printf '\n' >>/usr/lib/os-release
    echo 'VARIANT_ID="ryven-sericea"' >>/usr/lib/os-release
  fi
...
# /etc/sway/config already includes /usr/share/sway/config.d/*.conf, where
# the Ryven overlay ships — do NOT also copy it into /etc/sway/config.d or
# sway parses every directive twice (double swayidle etc.). The directory
# stays for admin drop-ins.
mkdir -p /etc/sway/config.d

# Ship the Quickshell bar (replaces waybar). quickshell's default config is
# ~/.config/quickshell/shell.qml, so seed skel from the whole XDG config
# tree (shell.qml + ryven/ modules + extensions/ drop-ins + AGENTS.md for
# AI-assisted editing with pi). Fail with a clear message when the overlay
# source moved instead of a bare cp error.
