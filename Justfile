set dotenv-filename := "ryven.env"
set dotenv-load := true

# IMAGE_NAME is the product name (Ryven). OCI/podman refs must be lowercase.

export image_title := env_var("IMAGE_NAME")
export image_name := lowercase(env_var("IMAGE_NAME"))
...
check:
    ...
    echo "Checking syntax: system_files/usr/share/ryven/justfile"
    just --unstable --fmt --check -f system_files/usr/share/ryven/justfile
...
build $target_image=image_name $tag=default_tag:
    ...
    if [[ -z "$(git status -s)" ]]; then
        ...
        LABELS+=("--label" "io.artifacthub.package.readme-url=https://raw.githubusercontent.com/{{ repo_organization }}/{{ image_title }}/${GIT_SHA}/README.md")
        LABELS+=("--label" "org.opencontainers.image.documentation=https://raw.githubusercontent.com/{{ repo_organization }}/{{ image_title }}/${GIT_SHA}/README.md")
        LABELS+=("--label" "org.opencontainers.image.source=https://github.com/{{ repo_organization }}/{{ image_title }}/blob/${GIT_SHA}/Containerfile")
        LABELS+=("--label" "org.opencontainers.image.url=https://github.com/{{ repo_organization }}/{{ image_title }}/tree/${GIT_SHA}")
    ...
    # Ephemeral CI token for GitHub API lookups inside the build (secret mount,
    # never a layer). Local builds without it stay anonymous.
...
# Build the Anaconda installer container (installer/Containerfile) used by
# bootc-installer ISOs. BASE_IMAGE must be in rootful storage first.
_build-installer-container $target_image $tag: (_rootful_load_image target_image tag)
    #!/usr/bin/env bash
    set -euox pipefail
    sudo podman build \
      --pull=newer \
      --build-arg BASE_IMAGE="{{ target_image }}:{{ tag }}" \
      -t "{{ target_image }}-installer:{{ tag }}" \
      -f installer/Containerfile .

# Build an installer ISO with bootc-image-builder's bootc-installer type
# (the new Fedora installer: Anaconda + Web UI). The installer container
# provides the Anaconda environment; the payload ref is what gets installed.
_build-bib-installer $target_image $tag: (_build-installer-container target_image tag)
    #!/usr/bin/env bash
    set -euox pipefail
    BUILDTMP=$(mktemp -p "${PWD}" -d -t _build-iso.XXXXXXXXXX)
    sudo podman run \
      --rm \
      -it \
      --privileged \
      ...
      --rootfs=btrfs \
      --use-librepo=True \
      --installer-payload-ref "{{ target_image }}:{{ tag }}" \
      "{{ target_image }}-installer:{{ tag }}"
    mkdir -p output
    sudo mv -f $BUILDTMP/* output/
    sudo rmdir $BUILDTMP
    sudo chown -R $USER:$USER output/

# Build an installer ISO (new Fedora installer)
[group('Build Virtal Machine Image')]
build-iso $target_image=("localhost/" + image_name) $tag=default_tag: && (_build-bib-installer target_image tag)
...
# Rebuild an installer ISO (new Fedora installer)
[group('Build Virtal Machine Image')]
rebuild-iso $target_image=("localhost/" + image_name) $tag=default_tag: (build target_image tag) && (_build-bib-installer target_image tag)
...
[group('Run Virtal Machine')]
run-vm-iso $target_image=("localhost/" + image_name) $tag=default_tag: && (_run-vm target_image tag "iso" "disk_config/installer.toml")
