...
# Fetch the latest BetterBird x86_64 release into the build context
# (_build/betterbird/) on the CI host, so the image build installs it
# from disk (/ctx) instead of fetching inside the container. The
# in-container fetch to betterbird.eu / its BunnyCDN mirror has
# repeatedly killed the build container without a trace; a host-side
# fetch has full log visibility and fails hard and visibly. Weekly CI
# rebuilds keep the published image on the current release.
fetch-betterbird:
    #!/usr/bin/env bash

    set -euox pipefail

    # CDN first (a CDN built for bulk downloads; the origin sits on small
    # shared hosting that intermittently drops datacenter connections).
    SOURCES=(
    ...
    for base in "${SOURCES[@]}"; do
      # --max-time caps each attempt (a silently dropped connection must
      # not consume the whole retry budget).
    ...
    # Current release of each ESR line first ("-latest-" marker), then
    # every plain en-US x86_64 build. Exclude the Previous/ archive;
